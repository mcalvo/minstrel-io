...
function populateSegmentDropdown(){
    ...
    $.getJSON('/nations/segmentTypes', function(data){
...
// Ajax calls for our nationality dropdown
function populateNationDropdown(){
    var listContent = '';

    // jQuery AJAX call for json
    $.getJSON('/nations/nationality', function(data){
        nationsData = data.data;
        // Add row for each item in data
        $.each(data.data, function(){
            listContent += '<option value="' + this._id + '">' + this.text + '</option>'; // Segment
        });
        ...
        //Inject into HTML table
        $('#segAdd fieldset select#inputNation').html(listContent);
    });
};
...
function showSegment(event) {
    ...
    $('#segmentNationality').text(thisSegmentObject.nation.text);
}
...
function addSegment(event) {
    ...
    var newSeg = {
        ...
        'nation': $('#segAdd fieldset select#inputNation').val()
    }

    $.ajax({
        type: 'POST',
        data: newSeg,
        url: '/nations/addsegment',
        dataType: 'JSON'
...
function deleteSegment(event) {
    ...
    if( confirmation === true){
        $.ajax({
            type: 'DELETE',
            url: '/nations/deletesegment/'+ $(this).attr('rel'),
        }).done(function(response) {
            if (response.msg === ''){
            } else {
                alert('Error: ' + response.msg);
